...
COPY --chown=user:users osu osu
RUN python manage.py collectstatic --no-input

# Setup staticfiles and Nginx
USER root
RUN mkdir -p /var/www/osu/
RUN cp -a $APP/staticfiles/ /var/www/osu/static/
COPY deploy/nginx.conf /etc/nginx/sites-enabled/osu.conf
RUN mkdir -p /data
